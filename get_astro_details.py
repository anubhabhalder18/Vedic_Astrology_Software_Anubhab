...
def get_asc(chart: chart_details):
    ...
    asc_position = Planet.make("Ascendant", asc, 0.0)
    return asc_position
...
def get_planet(chart: chart_details, planet: str):
    ...
    jd_ut = swe.julday(2004, 3, 18, 22+4/60+44/3600-5.5)
    swe.set_sid_mode(swe.SIDM_TRUE_CITRA)
    ...
    if is_ketu:
        curr_long = (curr_long + 180) % 360  # true Ketu position

    next_info = swe.calc_ut(jd_ut + 1/86400, planet_id, swe.FLG_SIDEREAL)
    next_long = next_info[0][0]
    if is_ketu:
        next_long = (next_long + 180) % 360

    # --- Correct speed calculation ---
    diff = ((next_long - curr_long + 540) % 360) - 180
    speed = diff * 86400
