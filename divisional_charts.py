...
def make_d5_horoscope(d1: Horror_scope) -> Horror_scope:
    """D5 — Panchamsa chart (Parashari method) with special lagnas included."""
    ...
    def d5_long(longitude):
        ...
        # Panchamsa part index (each 6°)
        part = int(inside // 6)
        if part > 4:
            part = 4

        # Determine first Panchamsa sign
        if rashi in movable:
            first_sign = rashi                 # same sign
        elif rashi in fixed:
            first_sign = (rashi + 4) % 12      # 5th from
        else:
            first_sign = (rashi + 8) % 12      # 9th from

        # Panchamsa sign shift (direct order)
        new_rashi = (first_sign + part) % 12

        # Degree inside this 6° part mapped to 0–30°
        inside_part = inside - part * 6
        percent = inside_part / 6.0
        new_deg = percent * 30.0
    ...
    return Horror_scope(
        ...
        longitude=d1.longitude,
        latitude=d1.latitude,
        special_lagnas=d5_special_lagnas       # 🔥 EXACTLY LIKE YOUR D2
    )


def make_d5_hadda_horoscope(d1: Horror_scope) -> Horror_scope:
    """D5 — Hadda Panchamsha chart using 5 equal 6° divisions with odd/even mapping.
       (Most commonly used Panchamsha in modern software)
    """

    # Hadda Panchamsha mapping table
    # Each entry: (odd_sign_result, even_sign_result)
    # 0 = Aries ... 11 = Pisces
    HADDA_TABLE = {
        0:  (0, 1),     # 0–6°   → Aries / Taurus
        1:  (10, 5),    # 6–12°  → Aquarius / Virgo
        2:  (8, 11),    # 12–18° → Sagittarius / Pisces
        3:  (2, 9),     # 18–24° → Gemini / Capricorn
        4:  (6, 7),     # 24–30° → Libra / Scorpio
    }
    # part index: 0..4
    ...
    def d5_long(longitude):
        ...
        # Determine 6° part
        part = int(inside // 6)          # 0..4
        if part > 4:
            part = 4

        # Odd/Even rashi rule:
        if rashi % 2 == 0:               # odd sign indices
            new_rashi = HADDA_TABLE[part][0]
        else:                             # even sign indices
            new_rashi = HADDA_TABLE[part][1]

        # Degree inside 6° mapped proportionally to 0–30°
        degree_in_part = inside - (part * 6)
        pct = degree_in_part / 6.0
        new_deg = pct * 30.0

        return new_rashi * 30 + new_deg

    # Convert planet to D5
    def convert(pl: Planet):
        new_long = d5_long(pl.planet_position.longitude)
        return Planet.make(pl.name, new_long, pl.speed * 5)

    # Ascendant:
    asc_src = (d1.ascendant.longitude if not hasattr(d1.ascendant, "planet_position")
               else d1.ascendant.planet_position.longitude)
    asc_long = d5_long(asc_src)
    asc = Planet.make("Ascendant", asc_long, 0)
    ...
    return Horror_scope(
        ...
        special_lagnas=d5_special_lagnas
    )



def make_d2_male_female_horoscope(d1: Horror_scope) -> Horror_scope:


    # 0 = Aries ... 11 = Pisces
    KASHINATH_D2_HORA = {
        0: (0, 1),   # Aries:     0–15° -> Scorpio,     15–30° -> Aries
        1: (1, 6),   # Taurus:    0–15° -> Taurus,      15–30° -> Libra
        2: (2, 5),   # Gemini:    0–15° -> Virgo,       15–30° -> Gemini
        3: (3, 4),   # Cancer:    0–15° -> Cancer,      15–30° -> Leo
        4: (4, 3),   # Leo:       0–15° -> Leo,         15–30° -> Cancer
        5: (5, 2),   # Virgo:     0–15° -> Gemini,      15–30° -> Virgo
        6: (6, 1),   # Libra:     0–15° -> Libra,       15–30° -> Taurus
        7: (7, 0),   # Scorpio:   0–15° -> Aries,       15–30° -> Scorpio
        8: (8, 11),  # Sagittarius: 0–15° -> Pisces,    15–30° -> Sagittarius
        9: (9, 10),  # Capricorn: 0–15° -> Capricorn,   15–30° -> Aquarius
        10: (10, 9), # Aquarius:  0–15° -> Aquarius,    15–30° -> Capricorn
        11: (11, 8), # Pisces:    0–15° -> Sagittarius, 15–30° -> Pisces
    }
...
def make_varanasi_drekkana_horoscope(d1: Horror_scope) -> Horror_scope:
    """returns a new Horror_scope (D3 Varanasi Drekkana) derived from D1"""
    ...
    # -------------------------------
    # Varanasi Drekkana Sequences (0-based)
    # -------------------------------
    FIERY_SEQ  = [0, 1, 2]      # Aries → Taurus → Gemini
    EARTHY_SEQ = [3, 4, 5]      # Cancer → Leo → Virgo
    AIRY_SEQ   = [6, 7, 8]      # Libra → Scorpio → Sagittarius
    WATERY_SEQ = [9, 10, 11]   # Capricorn → Aquarius → Pisces
    ...
    def drekkana_long(longitude):
        ...
        part = int(inside // D3_PART)  # 0,1,2

        # -------- VARANASI RULE --------
        if rashi in FIERY:
            new_rashi = FIERY_SEQ[part]
        elif rashi in EARTHY:
            new_rashi = EARTHY_SEQ[part]
        elif rashi in AIRY:
            new_rashi = AIRY_SEQ[part]
        elif rashi in WATERY:
            new_rashi = WATERY_SEQ[part]
        else:
            new_rashi = rashi

        # map longitude proportionally inside new sign
        inside_part = inside - part * D3_PART     # 0–10°
        percent = inside_part / D3_PART           # 0.0–1.0
        new_deg = percent * 30                    # 0°–30° inside D3 sign
    ...
    return Horror_scope(
        ...
        longitude=d1.longitude,
        latitude=d1.latitude,
        special_lagnas=d3_special_lagnas   # 🔥 INCLUDED
    )
